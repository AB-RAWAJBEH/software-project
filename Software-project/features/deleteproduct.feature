...
Scenario: delete product successfully
	...
	And there is a product  name "cover" , type "water proof" , and the price "17", amount"1"
	When the product is deleted from the system
	Then the product with name "cover" ,type "water proof" , and the price "17", amount"1" off the system
	 
	
Scenario: delete product and admin is not logged in
    Given  the admin is not logged in
	And there is a product  name "cover" , type "water proof" , and the price "20", amount"1"
	When the product is deleted from the system
